...
- `bun install` —— 安装依赖
- `bun run build` —— 完整重建：catalog + knowledge + **modern css** + bookmarklet（**需要 `vendor/`，见下文**）
- `bun run build:bookmarklet` —— 仅从已提交的 `data/` 组装 `dist/`，无需私有数据
- `bun run build:css` —— 预编译旧 classic 面板的 Tailwind v3 到 `src/ui/styles.generated.mjs`（仅维护历史 classic 源时使用）
...
- `bun run build:css:modern` —— 用 **Tailwind v4**（`@tailwindcss/cli`，包别名 `tailwindcss4`）从 `src/ui/tailwind.modern.css` 预编译到 `src/ui/stylesModern.generated.mjs`。改了 modern 面板/图标类名后重跑；完整 `build` 会运行它。
...
- **图标统一用 SVG**：UI 内**不得出现 unicode emoji**，当前运行时图标走 `src/ui/iconsMicro.mjs`（Heroicons Micro 16px，同名语义键）；历史 classic 面板仍有自己的 `ICON` 注册表。
- **modern 是唯一运行时主题**：`main.mjs` 直接导入 `src/ui/panelModern.mjs`，不再支持 `/theme` 切换、设置页主题 select 或 `dist/modern.min.js` 懒加载包；旧 `localStorage m3e_theme` 会被写回 `modern` 以免历史配置误导。构成：`panelModern.mjs`（纯视图，视觉为 zinc+单蓝 / Heroicons Micro / 内嵌 Geist+Sarasa 字体）、`iconsMicro.mjs`、`stylesModern.generated.mjs`（Tailwind **v4** 产出，`tailwind.modern.css` 分层跳 preflight + `@custom-variant dark` 类驱动；v4 以 `tailwindcss4` 别名引入）、`fontsModern.generated.mjs`（Geist + Sarasa Gothic SC + Sarasa Mono SC 子集 3500 一级字 woff2 base64，**生成且提交**，OFL 见 `src/ui/OFL.txt`/`OFL-Sarasa.txt`；按 OFL 重命名 family 为 `M3E Geist`/`M3E Sarasa`/`M3E Sarasa Mono`）、`fontsModern.mjs`（FontFace 激活器，失败优雅降级 system-ui/monospace）。`build:css:modern` 进入完整 `build`；`build:fonts:modern` 仍为按需脚本（生成物已提交，同 term-font 惯例）。`blockTree.mjs` 的确认预览固定传 `MODERN_PALETTE`（折叠到 zinc + 单蓝，**无 indigo**）。历史 classic 文件（`panel.mjs` / `styles.generated.mjs` / `tailwind.config.cjs` / `tools/build-css.mjs`）当前不被运行时引用，除非明确恢复多主题，否则不要继续扩展。
