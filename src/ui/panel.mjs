...
export function createPanel(opts = {}) {
  ...
  root.innerHTML = `
    ...
    <div class="m3e isolate">
      <button data-act="reopen" class="m3e-launcher pointer-events-auto absolute right-0 top-[46%] hidden rounded-l-xl bg-blue-600 px-2 py-3.5 font-semibold tracking-[3px] text-white shadow-lg hover:bg-blue-500 [writing-mode:vertical-rl]">AI 编程</button>
      <div data-wrap class="m3e-wrap pointer-events-auto absolute flex flex-col overflow-hidden bg-white text-zinc-900 antialiased outline outline-1 outline-zinc-950/15 transition-[transform,opacity] duration-200 dark:bg-zinc-950 dark:text-zinc-100 dark:outline-white/10">
        <header data-drag class="flex items-center gap-2 border-b border-zinc-950/10 bg-zinc-100 px-3 py-2.5 dark:border-white/10 dark:bg-zinc-900">
          <b class="flex-1 truncate text-sm font-semibold tracking-tight">AI 图形化编程</b>
          <span data-board class="inline-flex items-center gap-1.5 whitespace-nowrap rounded-full bg-zinc-950/5 px-2 py-1 text-[11.5px] tabular-nums text-zinc-500 dark:bg-white/5 dark:text-zinc-400">检测中…</span>
          <button data-act="dock" type="button" title="切换浮窗/停靠" aria-label="切换浮窗/停靠" class="grid size-[30px] place-items-center rounded-lg text-zinc-500 hover:bg-zinc-950/5 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-white/5 dark:hover:text-zinc-100">${ICON.float}</button>
          <button data-act="settings" type="button" title="设置" aria-label="设置" class="grid size-[30px] place-items-center rounded-lg text-zinc-500 hover:bg-zinc-950/5 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-white/5 dark:hover:text-zinc-100">${ICON.gear}</button>
          <button data-act="toggle" type="button" title="收起" aria-label="收起" class="grid size-[30px] place-items-center rounded-lg text-zinc-500 hover:bg-zinc-950/5 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-white/5 dark:hover:text-zinc-100">${ICON.collapse}</button>
        </header>

        <div data-feed class="m3e-scroll flex flex-1 flex-col gap-3 overflow-y-auto px-3 py-3.5 text-[13px] leading-relaxed"></div>

        <div data-settings class="hidden flex-col gap-2.5 border-t border-zinc-950/10 bg-zinc-50 px-3 py-3 dark:border-white/10 dark:bg-zinc-900">
          <label class="text-xs text-zinc-500 dark:text-zinc-400" for="m3e-base">LLM Base URL（OpenAI 兼容）</label>
          <input id="m3e-base" name="baseURL" data-cfg="baseURL" placeholder="https://api.deepseek.com/v1" class="rounded-lg bg-white px-2.5 py-2 text-[12.5px] text-zinc-900 ring-1 ring-zinc-950/15 placeholder:text-zinc-400 focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-blue-500 dark:bg-black/40 dark:text-zinc-100 dark:ring-white/10 dark:placeholder:text-zinc-500">
          ...
          <button data-act="saveCfg" type="button" class="mt-1 self-end rounded-lg bg-blue-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-500">保存设置</button>
        </div>

        <div class="border-t border-zinc-950/10 bg-white dark:border-white/10 dark:bg-zinc-950">
          <div data-progress class="hidden border-b border-zinc-950/10 dark:border-white/10"></div>
          <div class="px-3 py-2.5">
            <div data-status class="mb-1.5 hidden text-[11.5px] tabular-nums text-zinc-500 dark:text-zinc-500"></div>
            <div class="flex items-end gap-2">
              <textarea data-input rows="2" placeholder="描述需求或提问，例如：按A键显示温度 / 把刚才那块改成红色（/help 查看命令）" class="m3e-scroll max-h-40 flex-1 resize-none rounded-xl bg-zinc-50 px-3 py-2 text-[13px] leading-relaxed text-zinc-900 ring-1 ring-zinc-950/15 placeholder:text-zinc-400 focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-blue-500 dark:bg-zinc-900 dark:text-zinc-100 dark:ring-white/10 dark:placeholder:text-zinc-500"></textarea>
              <button data-act="send" type="button" title="发送" class="grid size-9 shrink-0 place-items-center rounded-xl bg-blue-600 text-white hover:bg-blue-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-500 disabled:bg-zinc-950/5 disabled:text-zinc-400 dark:disabled:bg-white/5 dark:disabled:text-zinc-600">${ICON.send}</button>
  ...
  // Shared card surface (light base + dark). Tone tints the ring only.
  const cardCls = (tone = "") => {
    const ring = tone === "err"
      ? "ring-red-500/30"
      : tone === "ok"
      ? "ring-emerald-500/25 dark:ring-emerald-500/25"
      : "ring-zinc-950/10 dark:ring-white/10";
    return `overflow-hidden rounded-xl bg-zinc-50 ring-1 dark:bg-zinc-900/60 ${ring}`;
  };

  /** A tool-call card (think / search / edit / run). `icon` is a key into the
   *  ICON registry; unknown keys fall back to the help glyph. The card expands
   *  while running and auto-collapses to its one-line header once `setTone` marks
   *  it done (ok/err); the header stays clickable to re-expand. */
  function toolCard({ icon = "help", title, body = "", tone = "" }) {
    const card = div(cardCls(tone));
    card.innerHTML =
      `<button type="button" data-head class="flex w-full items-center gap-1.5 px-2.5 py-2 text-left text-[12px] font-medium text-zinc-700 hover:bg-zinc-950/[.03] dark:text-zinc-300 dark:hover:bg-white/[.03]">` +
        `<span class="text-zinc-500 dark:text-zinc-500">${ICON[icon] || ICON.help}</span>` +
        `<span data-title class="flex-1 truncate">${esc(title)}</span>` +
        `<span data-chev class="text-zinc-400 transition-transform duration-150 dark:text-zinc-500">${ICON.chevron}</span>` +
      `</button>` +
      `<div data-wrapbody class="px-2.5 pb-2.5"><div data-body class="text-[12px] text-zinc-600 dark:text-zinc-400"></div></div>`;
    ...
    return {
      ...
      setBody(text, mono = false) {
        ...
        b.className = mono
          ? "m3e-scroll max-h-48 overflow-auto whitespace-pre-wrap rounded-md bg-zinc-100 p-2 font-mono text-[11px] text-zinc-700 dark:bg-black/40 dark:text-zinc-300"
  ...
  function ask({ question, options = [], multi = false }) {
    return new Promise((resolve) => {
      const card = div("rounded-xl bg-blue-500/10 p-2.5 ring-1 ring-blue-500/30");
      const opts = options.map((o, i) => {
        const desc = o.description ? `<div class="mt-0.5 text-[11px] text-zinc-500 dark:text-zinc-400">${esc(o.description)}</div>` : "";
        return `<button type="button" data-o="${i}" aria-pressed="false" class="w-full rounded-lg bg-zinc-950/[.04] px-2.5 py-1.5 text-left text-[12px] text-zinc-700 ring-1 ring-transparent hover:bg-zinc-950/[.07] aria-pressed:bg-blue-600/20 aria-pressed:ring-blue-500/40 dark:bg-white/5 dark:text-zinc-200 dark:hover:bg-white/10 dark:aria-pressed:bg-blue-600/30"><span class="font-medium text-zinc-900 dark:text-zinc-100">${esc(o.label)}</span>${desc}</button>`;
      }).join("");
      card.innerHTML =
        `<div class="text-[12.5px] font-medium text-blue-700 dark:text-blue-200">${esc(question)}</div>` +
        `<div data-opts class="mt-2 flex flex-col gap-1.5">${opts}</div>` +
        (multi ? `<button type="button" data-submit class="mt-2 self-end rounded-lg bg-blue-600 px-2.5 py-1 text-[12px] font-medium text-white hover:bg-blue-500 disabled:bg-zinc-950/5 disabled:text-zinc-400 dark:disabled:bg-white/5 dark:disabled:text-zinc-600" disabled>提交</button>` : "");
      append(card);
      ...
      const finish = (labels) => {
        ...
        card.innerHTML = `<div class="px-2.5 py-2 text-[12px] text-zinc-500 dark:text-zinc-400">${esc(question)} — ${esc(shown)}</div>`;
  ...
  function confirm({ title, detail }) {
    return new Promise((resolve) => {
      const card = div("rounded-xl bg-amber-500/10 p-2.5 ring-1 ring-amber-500/30");
      card.innerHTML =
        `<div class="text-[12.5px] font-medium text-amber-700 dark:text-amber-200">${esc(title)}</div>` +
        (detail ? `<div class="mt-1 text-[12px] text-zinc-700 dark:text-zinc-300">${esc(detail)}</div>` : "") +
        `<div class="mt-2 flex gap-2">
          <button type="button" data-c="once" class="rounded-lg bg-blue-600 px-2.5 py-1 text-[12px] font-medium text-white hover:bg-blue-500">允许一次</button>
          ...
          <button type="button" data-c="no" class="rounded-lg bg-zinc-950/[.05] px-2.5 py-1 text-[12px] text-zinc-600 hover:bg-zinc-950/10 dark:bg-white/5 dark:text-zinc-300 dark:hover:bg-white/10">拒绝</button>
        </div>`;
      append(card);
      card.addEventListener("click", (e) => {
        ...
        card.className = cardCls();
        card.innerHTML = `<div class="px-2.5 py-2 text-[12px] text-zinc-500 dark:text-zinc-400">${esc(title)} — ${label}</div>`;
  ...
  const setBoard = (text, cls = "") => {
    ...
    boardEl.className = `inline-flex items-center gap-1.5 whitespace-nowrap rounded-full px-2 py-1 text-[11.5px] tabular-nums ${cls === "ok" ? "bg-emerald-500/15 text-emerald-700 dark:text-emerald-300" : cls === "err" ? "bg-red-500/15 text-red-700 dark:text-red-300" : "bg-zinc-950/5 text-zinc-500 dark:bg-white/5 dark:text-zinc-400"}`;
  };
