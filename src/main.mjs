...
import { blockTreeHtml, MODERN_PALETTE } from "./ui/blockTree.mjs";
...
async function boot() {
  ...
  try { watchNight(caps, (dark) => { currentDark = dark; panel.setDark(dark); }); }
  ...
  function confirmEdit(title, args) {
    ...
    try {
      ...
      if (pre.ok) {
        ...
        preview = svg || blockTreeHtml(pre.postIR, data.catalog, bt);
      } else if (pre.rawIR?.length) {
        preview = blockTreeHtml(pre.rawIR, data.catalog, bt);
      }
    } catch (e) {
